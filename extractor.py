...
def OS_terminal_download():
    if IS_WINDOWS:
        return ["certutil", "-urlcache", "-split", "-f"]
    else:
        return ["curl", "-L"]
...
arg.add_argument("-his", "--history", action="store_true", required=False, help="Select from previously cached JSON files in jsoncache directory")
...
def select_cached_json(jsoncache_path):
    json_files = glob.glob(os.path.join(jsoncache_path, "*.json"))
    
    if not json_files:
        print("ERROR: No JSON files found in jsoncache directory")
        raise OSError("No cached JSON files available")
    
    json_files.sort()
    print("\nAvailable cached JSON files:")
    print("-" * 50)
    ...
    while True:
        try:
            choice = int(input(f"Select JSON file (1-{len(json_files)}): "))
            if 1 <= choice <= len(json_files):
                selected_file = json_files[choice - 1]
                print(f"INFO : Selected: {os.path.basename(selected_file)}")
                return selected_file
            else:
                print(f"ERROR: Please enter a number between 1 and {len(json_files)}")
        except ValueError:
            print("ERROR: Invalid input. Please enter a number.")
print("INFO : Checking for JSON files in the raw directory.")

if args["history"] and args["input"]:
    print("WARNING: Input file is ignored when using -his flag")

if args["history"]:
    json_data = [select_cached_json(jsoncache_dir)]
elif not args["input"]:
...
if not os.path.exists(os.path.join(basepath, "genome", "Homo_sapiens.GRCh38.fa")):
    ...
    cmd_download = [URL, gz_file] if IS_WINDOWS else [URL, "-o", gz_file]
...
if strand == -1:
    ...
    target = genome[chrom][prom_start:prom_end]
...
if args["output"]:
    ...
    output_name = None
def remove_exons(js_data, genome, full_seq, chromosome, sstrand):
    ...
    for i in range(len(Exons)):
        ...
        for j in range(len(t_exons)):
            ...
            if sstrand == -1:
                seq_target = genome[chromosome][coords['start']-1:coords['end']]
            else:
                seq_target = genome[chromosome][coords['end']-1:coords['start']]
            seq = seq_target.seq if sstrand == 1 else seq_target.reverse.complement.seq
            raw_targets[t_key][exon_out_key] = seq.upper() 
    

    removed_seqs = {}
    for t_key, exons in raw_targets.items():
        removed_seq = full_seq.upper()
        for exon_key, exon_seq in exons.items():
            removed_seq = removed_seq.replace(exon_seq, "")
        removed_seqs[t_key] = removed_seq

    return removed_seqs


removed_exon_seqs = remove_exons(gene_location, genome, fasta_seq, chrom, strand)

def write_rmexon(removed_seqs_dict, js_dict, chrom, strand, prom_start, prom_end, output_name=None):
    sorted_keys = sorted(
    ...
    if output_name:
        tar_gz_path = output_name.replace('.fasta', '') + '_removed-exons.tar.gz'
    else:
...
def write_norepeats(full_seq,chrom, strand, prom_start, prom_end, output_name=None):
    cleaned_seq = ''.join([c for c in full_seq if not c.islower()])
    if output_name:
        out_path = output_name.replace('.fasta', '') + '_norepeats.fasta'
    else:
...
def write_canonical(json_data, genome, chromomosome, strand, upstream, downstream, output_name=None):
    canonical_seq = None
    ...
    if canonical_seq:
        if output_name:
            out_path = output_name.replace('.fasta', '') + '_canonical.fasta'
        else:
...
def write_all(full_seq,chrom, strand, prom_start, prom_end, output_name=None):
    if output_name:
...
if args["mode"] == "no-exons":
    write_rmexon(removed_exon_seqs, gene_location, chrom, strand, prom_start, prom_end, output_name)
elif args["mode"] == "no-repeats":
    write_norepeats(fasta_seq, chrom, strand, prom_start, prom_end, output_name)
elif args["mode"] == "canonical":
    write_canonical(gene_location, genome, chrom, strand, upstream, downstream, output_name)
elif args["mode"] == "all":
    write_all(fasta_seq, chrom, strand, prom_start, prom_end, output_name)
else:
